...
class GraspedPredicate(UnsampleablePredicate, BinaryAtomicFormula):
    """
    Custom predicate: (grasped agent.n.01_1 object.n.01_1)
    Checks whether the robot (entity1) is currently holding the object (entity2) in any arm.
    Not sampleable — the BDDLSampler cannot set this state; it is only used for evaluation.
    """

    STATE_NAME = "grasped"

    def _evaluate(self, entity1, entity2, **kwargs):
        robot = entity1.wrapped_obj
        obj = entity2.wrapped_obj
        if robot is None or obj is None or not entity1.exists or not entity2.exists:
            return False
        if hasattr(robot, "_ag_obj_in_hand") and hasattr(robot, "arm_names"):
            return any(robot._ag_obj_in_hand.get(arm) == obj for arm in robot.arm_names)
        return False


class HandOverPredicate(UnsampleablePredicate, BinaryAtomicFormula):
    """
    Custom predicate: (hand_over agent.n.01_1 object.n.01_1)
    Checks whether the object has been transferred from the arm that initially grasped it
    to the OTHER arm, and is currently held by that other arm (the initial arm no longer
    holds it).

    Semantics (S1a, strict, any swap direction): the initial grasping arm is recorded ONCE
    on the first _evaluate call of the episode (step-0, right after state restore). Success =
    the object is currently held by an arm different from the initial arm, the initial arm no
    longer holds it, and it is still held (must-still-hold at goal time).

    Self-contained: history lives on the predicate instance (no OmniGibson core change). Relies
    on _evaluate being called every step (PredicateGoal._step) and at step-0 (trivial-success
    check in subtask_eval). The same instance is reused across episodes of a skill -- safe
    because every episode starts from the same snapshot (consistent initial arm).

    Convention: entity1 is the agent (e.g. agent.n.01_1); entity2 is the object to hand over.
    Caveat: if step-0 has not recovered the grasp (initial arm recorded as None), this skill
    instance can never succeed; a warning is logged so misconfigured snapshots can be detected.
    """

    STATE_NAME = "hand_over"

    def _evaluate(self, entity1, entity2, **kwargs):
        robot = entity1.wrapped_obj  # the agent/robot
        obj = entity2.wrapped_obj    # the object to hand over
        if robot is None or obj is None or not entity1.exists or not entity2.exists:
            return False
        if not (hasattr(robot, "_ag_obj_in_hand") and hasattr(robot, "arm_names")):
            return False

        # Arm currently holding obj (same comparison style as GraspedPredicate)
        current_arm = None
        for arm in robot.arm_names:
            if robot._ag_obj_in_hand.get(arm) == obj:
                current_arm = arm
                break

        # Stable object identity across steps; lazy-init instance-level history
        key = getattr(obj, "prim_path", None) or id(obj)
        if not hasattr(self, "_ho_initial_arm"):
            self._ho_initial_arm = {}
        if key not in self._ho_initial_arm:
            # First observation (episode step-0): lock the initial grasping arm
            self._ho_initial_arm[key] = current_arm
            if current_arm is None:
                log.warning(
                    f"[HandOver] object={getattr(obj, 'name', '?')}: no initial holding arm was detected at step 0. "
                    f"Success cannot be determined. Please verify grasp restoration in the scene snapshot."
                )
            else:
                log.debug(f"[HandOver] INIT  object={getattr(obj, 'name', '?')}  initial_arm={current_arm}")

        initial_arm = self._ho_initial_arm[key]

        # S1a strict: object must have LEFT the initial arm and now be held by a different arm
        initial_still_holds = (
            initial_arm is not None and robot._ag_obj_in_hand.get(initial_arm) == obj
        )
        success = (
            initial_arm is not None
            and current_arm is not None
            and current_arm != initial_arm
            and not initial_still_holds
        )
        log.debug(
            f"[HandOver] {'PASS' if success else 'FAIL'}  object={getattr(obj, 'name', '?')}  "
            f"initial_arm={initial_arm}  current_arm={current_arm}  initial_still_holds={initial_still_holds}"
        )
        return success


class NearPredicate(UnsampleablePredicate, BinaryAtomicFormula):
    """
    Custom predicate: (near entity1 entity2)
    Checks whether entity1 (the agent/robot) is within DISTANCE_THRESHOLD meters of
    entity2's body (AABB) on the XY plane. Distance is measured from the robot's center
    to the nearest edge of entity2's axis-aligned bounding box (0 when overlapping),
    so that large objects whose center is far from their edge are handled correctly.
    Intended for navigation (move_to) skill success evaluation.
    Not sampleable — only used for goal evaluation, not BDDLSampler.

    Convention: entity1 is the agent (e.g. agent.n.01_1); entity2 is the target object.
    Usage example in skill YAML bddl_goal:
        "(near agent.n.01_1 wood_fireplace.n.01_1)"
    """

    STATE_NAME = "near"
    DISTANCE_THRESHOLD = 1.5  # meters, XY-plane only

    def _evaluate(self, entity1, entity2, **kwargs):
        import numpy as np

        obj1 = entity1.wrapped_obj  # the agent/robot
        obj2 = entity2.wrapped_obj  # the target object
        if obj1 is None or obj2 is None or not entity1.exists or not entity2.exists:
            return False

        # Robot center on the XY plane (entity1 is the agent by convention)
        p = np.array(obj1.get_position()[:2])

        # Target object AABB on the XY plane
        try:
            lo, hi = obj2.aabb
        except Exception:
            return False
        lo_xy = np.array(lo[:2].tolist())
        hi_xy = np.array(hi[:2].tolist())

        # Point-to-box distance on the XY plane:
        # 0 when the robot center lies inside the object's AABB footprint (overlap),
        # otherwise the distance to the nearest edge of the box.
        dx = max(lo_xy[0] - p[0], 0.0, p[0] - hi_xy[0])
        dy = max(lo_xy[1] - p[1], 0.0, p[1] - hi_xy[1])
        dist = float(np.hypot(dx, dy))

        log.debug(
            f"[Near] {'PASS' if dist <= self.DISTANCE_THRESHOLD else 'FAIL'}  "
            f"agent={getattr(obj1, 'name', '?')}  target={getattr(obj2, 'name', '?')}  "
            f"dist_xy={dist:.3f}  threshold={self.DISTANCE_THRESHOLD}  "
            f"target_aabb_xy=({lo_xy.round(3).tolist()}, {hi_xy.round(3).tolist()})"
        )
        return dist <= self.DISTANCE_THRESHOLD


class NearParticlesPredicate(UnsampleablePredicate, BinaryAtomicFormula):
    """
    Custom predicate: (near_particles entity1 entity2)
    Checks whether entity1 (the agent/robot) is within DISTANCE_THRESHOLD meters of
    the nearest particle of entity2 (a particle system, e.g. mud, water) on the XY plane.

    Unlike NearPredicate (which uses AABB point-to-box distance and fails for particle
    systems that lack an ``aabb`` property), this predicate retrieves all particle
    positions via ``get_particles_position_orientation()`` and computes the minimum XY
    Euclidean distance from the robot center to any individual particle.  This avoids
    false positives that would arise from computing a single large AABB over scattered
    particle groups.

    Intended for navigation (move_to) skill success evaluation when the target is a
    particle system (substance synset like mud.n.03_1).
    Not sampleable — only used for goal evaluation, not BDDLSampler.

    Convention: entity1 is the agent (e.g. agent.n.01_1); entity2 is the particle system.
    Usage example in skill YAML bddl_goal:
        "(near_particles agent.n.01_1 mud.n.03_1)"
    """

    STATE_NAME = "near_particles"
    DISTANCE_THRESHOLD = 1.5  # meters, XY-plane only
    ...
    def _evaluate(self, entity1, entity2, **kwargs):
        ...
        obj1 = entity1.wrapped_obj  # the agent/robot
        obj2 = entity2.wrapped_obj  # the particle system (e.g. mud)
        if obj1 is None or obj2 is None or not entity1.exists or not entity2.exists:
            return False
        ...
        try:
            # Retrieve all particle positions: (N, 3) tensor of world coordinates
            positions = obj2.get_particles_position_orientation()[0]
            if positions.numel() == 0:
                return False
            # XY coordinates of all particles → (N, 2) numpy array
            particle_xy = positions[:, :2].cpu().numpy()
        except Exception:
            return False

        # Minimum XY Euclidean distance from robot center to any particle
        diff = particle_xy - p  # (N, 2)
        dists = np.sqrt((diff ** 2).sum(axis=1))  # (N,)
        dist = float(dists.min())

        log.debug(
            f"[NearParticles] {'PASS' if dist <= self.DISTANCE_THRESHOLD else 'FAIL'}  "
            f"agent={getattr(obj1, 'name', '?')}  system={getattr(obj2, 'name', '?')}  "
            f"dist_xy={dist:.3f}  threshold={self.DISTANCE_THRESHOLD}  "
            f"n_particles={len(particle_xy)}"
        )
        return dist <= self.DISTANCE_THRESHOLD


class OverhangingEdgePredicate(UnsampleablePredicate, BinaryAtomicFormula):
    """
    Custom predicate: (overhanging_edge obj surface)
    True when obj hangs over the edge of surface on the XY plane:
      - obj still overlaps surface by >= OVERLAP_MIN in both X and Y (not fallen off)
      - obj extends beyond at least one edge of surface by >= OVERHANG_MIN

    Intended for push skill success evaluation: newspaper pushed to table edge.
    Combine with (ontop obj surface) in BDDL to ensure the object is still resting on the surface:
        "(and (ontop newspaper.n.03_1 table.n.02_1) (overhanging_edge newspaper.n.03_1 table.n.02_1))"

    Not sampleable — only used for goal evaluation, not BDDLSampler.
    """

    STATE_NAME = "overhanging_edge"
    OVERHANG_MIN = 0.05   # meters: obj must extend beyond surface edge by at least this much
    OVERLAP_MIN = 0.05    # meters: obj must still overlap with surface by at least this much

    def _evaluate(self, entity1, entity2, **kwargs):
        import numpy as np

        obj = entity1.wrapped_obj
        surface = entity2.wrapped_obj
        if obj is None or surface is None or not entity1.exists or not entity2.exists:
            return False

        try:
            obj_lo, obj_hi = obj.aabb        # (aabb_lo, aabb_hi), each shape (3,) tensor
            surf_lo, surf_hi = surface.aabb
        except Exception:
            return False

        # XY plane only
        obj_lo_xy = np.array(obj_lo[:2].tolist())
        obj_hi_xy = np.array(obj_hi[:2].tolist())
        surf_lo_xy = np.array(surf_lo[:2].tolist())
        surf_hi_xy = np.array(surf_hi[:2].tolist())

        # Overlap check: obj and surface must overlap in BOTH X and Y by >= OVERLAP_MIN
        overlap_x = min(obj_hi_xy[0], surf_hi_xy[0]) - max(obj_lo_xy[0], surf_lo_xy[0])
        overlap_y = min(obj_hi_xy[1], surf_hi_xy[1]) - max(obj_lo_xy[1], surf_lo_xy[1])
        has_overlap = overlap_x >= self.OVERLAP_MIN and overlap_y >= self.OVERLAP_MIN

        # Overhang check: obj extends beyond at least one edge of surface by >= OVERHANG_MIN
        # Compute actual overhang amount for each direction (positive = overhang, negative = inside)
        overhang_right = obj_hi_xy[0] - surf_hi_xy[0]   # right (+X)
        overhang_left  = surf_lo_xy[0] - obj_lo_xy[0]   # left  (-X)
        overhang_front = obj_hi_xy[1] - surf_hi_xy[1]   # front (+Y)
        overhang_back  = surf_lo_xy[1] - obj_lo_xy[1]   # back  (-Y)
        max_overhang = max(overhang_right, overhang_left, overhang_front, overhang_back)
        has_overhang = max_overhang >= self.OVERHANG_MIN

        result = has_overlap and has_overhang
        if result:
            # Log AABB details when condition passes, to help diagnose visual vs AABB discrepancy
            log.debug(
                f"[OverhangingEdge] PASS  obj={obj.name}  surface={surface.name}\n"
                f"  obj    AABB XY: lo={obj_lo_xy.round(3)}  hi={obj_hi_xy.round(3)}  "
                f"size_x={obj_hi_xy[0]-obj_lo_xy[0]:.3f}  size_y={obj_hi_xy[1]-obj_lo_xy[1]:.3f}\n"
                f"  surface AABB XY: lo={surf_lo_xy.round(3)}  hi={surf_hi_xy.round(3)}  "
                f"size_x={surf_hi_xy[0]-surf_lo_xy[0]:.3f}  size_y={surf_hi_xy[1]-surf_lo_xy[1]:.3f}\n"
                f"  overhang: right={overhang_right:.3f}  left={overhang_left:.3f}  "
                f"front={overhang_front:.3f}  back={overhang_back:.3f}  max={max_overhang:.3f}  "
                f"threshold={self.OVERHANG_MIN}"
            )
        return result


class SweepCleanPredicate(UnsampleablePredicate, BinaryAtomicFormula):
    """
    Custom predicate: (sweep_clean surface.n.01_1 substance.n.01_1)

    Returns True when the number of substance particles on the surface has strictly
    decreased compared to the initial count recorded at step-0 of this skill evaluation.

    Solves the step-0 false positive of (covered surface substance): `covered` is binary
    (>=1 particle -> True), so intermediate sweeps that partially remove particles don't
    change it -> the condition is trivially satisfied at step-0. `sweep_clean` instead
    checks for a decrease in particle count, ensuring actual sweeping occurred.

    Step-0 recording pattern follows HandOverPredicate: the initial particle count is
    locked on the first _evaluate call (episode step-0), and subsequent calls compare
    the current count against this value. The same instance is reused across episodes of
    a skill -- safe because every episode starts from the same snapshot.

    Convention: entity1 is the surface (e.g. floor.n.01_1); entity2 is the substance
    (e.g. mud.n.03_1, a particle system).
    """

    STATE_NAME = "sweep_clean"

    def _evaluate(self, entity1, entity2, **kwargs):
        surface = entity1.wrapped_obj   # e.g. floor.n.01_1
        system = entity2.wrapped_obj     # e.g. mud.n.03_1 (particle system)

        if surface is None or system is None or not entity1.exists or not entity2.exists:
            return False

        current_count = self._get_particle_count(surface, system)

        # Step-0 recording pattern (same as HandOverPredicate)
        key = getattr(surface, "prim_path", None) or id(surface)
        if not hasattr(self, "_sc_initial_count"):
            self._sc_initial_count = {}
        if key not in self._sc_initial_count:
            self._sc_initial_count[key] = current_count
            log.debug(
                f"[SweepClean] INIT  surface={getattr(surface, 'name', '?')}  "
                f"system={getattr(system, 'name', '?')}  initial_count={current_count}"
            )
            return False  # step-0: nothing removed yet

        initial_count = self._sc_initial_count[key]
        success = current_count < initial_count
        log.debug(
            f"[SweepClean] {'PASS' if success else 'FAIL'}  "
            f"surface={getattr(surface, 'name', '?')}  system={getattr(system, 'name', '?')}  "
            f"initial={initial_count}  current={current_count}"
        )
        return success

    def _get_particle_count(self, surface, system):
        """Get the number of particles of `system` on/attached to `surface`."""
        ...
        try:
            ...
            if scene.is_visual_particle_system(system_name=system.name):
                group_name = VisualParticleSystem.get_group_name(obj=surface)
                if group_name in system.groups:
                    return system.num_group_particles(group=group_name)
                return 0
            elif scene.is_physical_particle_system(system_name=system.name):
                return len(surface.states[ContactParticles].get_value(system))
            return 0
        except Exception:
            return 0


class AwayPredicate(UnsampleablePredicate, UnaryAtomicFormula):
    """
    Custom predicate: (away object.n.01_1)

    Returns True when the object has moved beyond MOVE_THRESHOLD meters (XY plane)
    from its initial position recorded at step-0 of this skill evaluation.

    Solves the push-to step false positive of (not (near agent object)):
    - `not near` can be satisfied by the agent backing away without pushing the object
    - `not near` requires the object to be >1.5m from the agent, which may be too far
    - During pushing, the robot is in contact with the object, so `near` stays True

    `away` instead checks that the OBJECT itself moved from its original position,
    ensuring a push actually occurred. Uses a smaller, more appropriate threshold.

    Step-0 recording pattern follows HandOverPredicate: the initial position is
    locked on the first _evaluate call, and subsequent calls compare the current
    position against this value.

    Convention: entity is the object that should have been pushed (e.g. lawn_chair.n.01_1).
    """

    STATE_NAME = "away"
    MOVE_THRESHOLD = 0.3  # meters, XY plane only

    def _evaluate(self, entity, **kwargs):
        import numpy as np

        obj = entity.wrapped_obj
        if obj is None or not entity.exists:
            return False

        pos = obj.get_position()
        current_pos = np.array([float(pos[0]), float(pos[1])], dtype=float)

        # Step-0 recording pattern (same as HandOverPredicate)
        key = getattr(obj, "prim_path", None) or id(obj)
        if not hasattr(self, "_away_initial_pos"):
            self._away_initial_pos = {}
        if key not in self._away_initial_pos:
            self._away_initial_pos[key] = current_pos
            log.debug(
                f"[Away] INIT  object={getattr(obj, 'name', '?')}  "
                f"initial_pos={current_pos.round(3).tolist()}"
            )
            return False  # step-0: hasn't moved yet

        initial_pos = self._away_initial_pos[key]
        dist = float(np.linalg.norm(current_pos - initial_pos))
        success = dist >= self.MOVE_THRESHOLD
        log.debug(
            f"[Away] {'PASS' if success else 'FAIL'}  "
            f"object={getattr(obj, 'name', '?')}  "
            f"initial={initial_pos.round(3).tolist()}  "
            f"current={current_pos.round(3).tolist()}  "
            f"dist={dist:.3f}  threshold={self.MOVE_THRESHOLD}"
        )
        return success
...
SUPPORTED_PREDICATES = {
    ...
    # Custom predicates for skill evaluation
    "grasped": GraspedPredicate,
    "hand_over": HandOverPredicate,
    "near": NearPredicate,
    "near_particles": NearParticlesPredicate,
    "overhanging_edge": OverhangingEdgePredicate,
    "sweep_clean": SweepCleanPredicate,
    "away": AwayPredicate,
}
