...
class BehaviorTask(BaseTask):
    ...
    def assign_object_scope_with_cache(self, env):
        ...
        # Merge any extra inst→name mappings injected via task config
        # (used when a BDDL object instance is not part of the original problem0.bddl
        # but is needed for skill evaluation, e.g. countertop.n.01_1 → countertop_kelker_0)
        extra_inst_to_name = env.task_config.get("extra_inst_to_name", {})
        if extra_inst_to_name:
            inst_to_name = dict(inst_to_name or {})
            inst_to_name.update(extra_inst_to_name)

        # Assign object_scope based on a cached scene
        for obj_inst in self.object_scope:
            if obj_inst in self.future_obj_instances:
                entity = None
            elif obj_inst.endswith("_*"):
                # Wildcard instance (e.g. tree.n.01_*): not stored literally in cache.
                # Find the first matching real instance by prefix and use it as a proxy,
                # or fall back to None if none found. Wildcard entries only appear in the
                # objects/init sections and are never referenced in skill goal conditions.
                prefix = obj_inst[:-1]  # e.g. 'tree.n.01_'
                matching = [k for k in inst_to_name if k.startswith(prefix)]
                if matching:
                    name = inst_to_name[matching[0]]
