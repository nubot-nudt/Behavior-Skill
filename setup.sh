...
Options:
  ...
  --new-env               Create a new conda environment 'behavior-skill'
...
if [ "$NEW_ENV" = true ]; then
    echo "Creating conda environment 'behavior-skill'..."
    ...
    if conda env list | grep -q "^behavior-skill "; then
        echo ""
        echo "ERROR: Conda environment 'behavior-skill' already exists!"
        echo ""
    ...
    conda create -n behavior-skill python=3.10 -c conda-forge -y
    conda activate behavior-skill

    [[ "$CONDA_DEFAULT_ENV" != "behavior-skill" ]] && { echo "ERROR: Failed to activate environment"; exit 1; }
...
if [ "$NEW_ENV" = true ]; then echo "✓ Created conda environment 'behavior-skill'"; fi
...
echo ""
if [ "$NEW_ENV" = true ]; then echo "To activate: conda activate behavior-skill"; fi